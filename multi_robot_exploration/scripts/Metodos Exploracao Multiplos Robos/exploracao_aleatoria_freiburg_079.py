...
#---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def map_callback(self, msg):
        ...
        if self.last_goal_point == 0:
            ...
            for data in frontier_robot:
                ...
                if middle_point.point.x == self.last_goal_point.point.x and distance > 1.5:
                    check = 1
    ...
    def select_best_frontier(self,frontier_robot):
        ...
        middle_points = [data[2] for data in frontier_robot]

        # Escolha aleatoriamente um ponto médio das fronteiras
        random_middle_point = random.choice(middle_points)

        #check distance of goal_point 
        for data in frontier_robot:
            size, distance, middle_point = data
            if middle_point.point.x == random_middle_point.point.x and middle_point.point.y == random_middle_point.point.y:
                # Save distance of random middle point
                max_distance = distance

        return random_middle_point, max_distance
