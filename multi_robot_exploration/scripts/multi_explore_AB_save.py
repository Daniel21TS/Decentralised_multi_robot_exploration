...
#---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def map_callback(self, msg):
        ...
        # Send data do excel (Time && Entropy)
        current_time = rospy.Time.now()

        data_zz = (current_time.to_sec(), self.initial_entropy)

        self.info_tab.append(data_zz)

        if len((self.info_tab)) >= 10:
            data = self.info_tab
    ...
    def compute_density(self, frontier_robot, map_type):
        ...
        for data in frontier_robot:
            ...
            if map_type == 1:
                # Primeira coluna
                if size <= 4 and densidade_ponderada <= 0.06:
                    info_gain = 85
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                if size > 4 and size <= 8 and densidade_ponderada <= 0.06:
                    info_gain = 111
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                if size > 8 and densidade_ponderada <= 0.06:
                    info_gain = 124
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                # Segunda coluna
                if size <= 4 and densidade_ponderada > 0.06 and densidade_ponderada <= 0.12:
                    info_gain = 91
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                if size > 4 and size <= 8 and densidade_ponderada > 0.06 and densidade_ponderada <= 0.12:
                    info_gain = 129
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                if size > 8 and densidade_ponderada > 0.06 and densidade_ponderada <= 0.12:
                    info_gain = 141
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                # Terceira coluna
                if size <= 4 and densidade_ponderada > 0.12:
                    info_gain = 87
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                if size > 4 and size <= 8 and densidade_ponderada > 0.12:
                    info_gain = 147
                    table.append((info_gain, densidade_ponderada, middle_point, size))
                if size > 8 and densidade_ponderada > 0.12:
                    info_gain = 173
                    table.append((info_gain, densidade_ponderada, middle_point, size))
